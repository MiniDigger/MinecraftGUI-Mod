...
import minecraftgui.controllers.Screen;
...
public class UnicodeFont extends Font {
    ...
    @Override
    public void drawString(String text, int y, int x, int size, Color color) {
        org.newdawn.slick.UnicodeFont unicodeFonts[] = fonts.get(size+"-"+color.getRGB());
        ...
        if(unicodeFonts != null) {
            ...
            unicodeFonts[screen.getScaleFactor()].drawString(x * screen.getScaleFactor(), y * screen.getScaleFactor(), text);
            GL11.glScaled(1f * screen.getScaleFactor(), 1f * screen.getScaleFactor(), 1);
        }
    }
